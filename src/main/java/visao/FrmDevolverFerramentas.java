    ...
    private void JBCadastrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_JBCadastrarActionPerformed
        ...
        try {
            ...
            for (Emprestimo e : listaEmprestimo) {
                if (e.getDataDev() == null || e.getDataDev().isEmpty()) {
                    ativos.add(e);
                }
            }
            if (posicaoEmprestimo < 0 || posicaoEmprestimo >= ativos.size()) {
                JOptionPane.showMessageDialog(null, "Selecione um empréstimo para devolver.");
                return;
            }

            int emprestimoId = ativos.get(posicaoEmprestimo).getIdEmprestimo();
            try {
                RMIClient.getServico().devolverFerramenta(emprestimoId);
            ...
            JOptionPane.showMessageDialog(null, "Erro ao obter empréstimos: " + rex.getMessage());
        }
        ...
        try {
            ...
            for (Emprestimo objeto : listaEmprestimo) {
                if (objeto.getDataDev() == null || objeto.getDataDev().isEmpty()) {
                    // Encontra nomes
                    String nomeAmigo = "(desconhecido)";
                    String nomeFerramenta = "(desconhecido)";
                    for (Amigo a : listaAmigo) {
                        if (a.getIdAmigo() == objeto.getIdAmigo()) {
                            nomeAmigo = a.getNomeAmigo();
                            break;
                        }
                    }
                    for (Ferramenta f : listaFerramenta) {
                        if (f.getIdFerramentas() == objeto.getIdFerramentas()) {
                            nomeFerramenta = f.getNomeFerramentas();
                            break;
                        }
                    }
                    JCBEmprestimo.addItem(objeto.getIdEmprestimo() + " - " + nomeAmigo + " - " + nomeFerramenta);
                }
            }
        } catch (RemoteException rex) {
            JOptionPane.showMessageDialog(null, "Erro ao carregar empréstimos: " + rex.getMessage());
        }
    }
